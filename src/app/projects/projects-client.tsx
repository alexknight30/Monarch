...
import { createProjectForView } from "@/components/ui/create-entity";
...
type ProjectSort = (typeof SORT_OPTIONS)[number]["id"];

function filterByTab(projects: Project[], tab: ProjectTab): Project[] {
  if (tab === "yours") return projects.filter((p) => p.visibility === "Private");
  if (tab === "shared") return projects.filter((p) => p.visibility === "Shared");
  return projects;
}

function filterByQuery(projects: Project[], query: string): Project[] {
...
export default function ProjectsClient({ projects }: { projects: Project[] }) {
  ...
  const [tab, setTab] = useState<ProjectTab>("yours");
  ...
  const visible = useMemo(
    () => sortProjects(filterByQuery(filterByTab(projects, tab), query), sort),
    [projects, tab, query, sort],
  );
  const sortLabel = SORT_OPTIONS.find((o) => o.id === sort)?.label ?? "Last updated";
  ...
  const onNew = async () => {
    try {
      const created = await createProjectForView(viewId);
    ...
    return (
      <div className="flex min-h-0 flex-1 flex-col items-center overflow-y-auto pt-13 pb-16">
        <div className="flex w-240 min-h-0 flex-1 flex-col">
          ...
          <BlankEmptyPlus addLabel="New project" createKind="project" />
        </div>
      </div>
    );
  }
